...
export default class ManualAwardPanel extends Component {
  ...
  view() {
    const t = (k: string) => app.translator.trans('ygpynet-point-system.admin.manual.' + k);
        ...
        {this.bulkResult && (
          <div className="PointSystemAdmin-result">
            ...
            {this.bulkResult.queued ? (
              <p>{app.translator.trans('ygpynet-point-system.admin.manual.bulk_queued', { total: this.bulkResult.total })}</p>
            ) : (
              [
                <p>
  ...
  async submit() {
    ...
    try {
      ...
      if (!userData) {
        app.alerts.show({ type: 'error' }, app.translator.trans('ygpynet-point-system.admin.manual.user_not_found'));
        return;
      }
      ...
      app.alerts.show({ type: 'success' }, app.translator.trans('ygpynet-point-system.admin.manual.success'));
    } catch (e: any) {
  ...
  async submitBulk() {
    ...
    try {
      ...
      app.alerts.show(
        { type: 'success' },
        this.bulkResult?.queued
          ? app.translator.trans('ygpynet-point-system.admin.manual.bulk_queued', { total: this.bulkResult.total })
          : app.translator.trans('ygpynet-point-system.admin.manual.bulk_success')
      );
    } catch (e: any) {
      app.alerts.show({ type: 'error' }, e?.response?.errors?.[0]?.detail || 'Failed');
    } finally {
