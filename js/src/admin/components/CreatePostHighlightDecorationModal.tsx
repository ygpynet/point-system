...
export default class CreatePostHighlightDecorationModal extends Modal {
  ...
  title() {
    return app.translator.trans('ygpynet-point-system.admin.post_hl.create_title');
  }
  ...
  content() {
    ...
    const t = (k: string) => app.translator.trans('ygpynet-point-system.admin.post_hl.' + k);
        ...
        <div className="Form-group">
          ...
          <select className="FormControl" value={draft.preset} onchange={(e: Event) => (draft.preset = (e.target as HTMLSelectElement).value)}>
            {BUILTIN_PRESETS.map((p) => (
              <option value={p}>{app.translator.trans(`ygpynet-point-system.admin.post_hl.preset_${p}`)}</option>
            ))}
  ...
  async commit() {
    ...
    try {
      ...
      app.alerts.show({ type: 'success' }, app.translator.trans('ygpynet-point-system.admin.post_hl.created'));
      this.hide();
    } catch (e: any) {
