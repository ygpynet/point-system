    ...
    return (
      <div className="PointSystemAdmin-section">
        <div className="PointSystemAdmin-section-header PointSystemAdmin-section-header--withAction">
          <div>
            <h2>{app.translator.trans('ygpynet-point-system.admin.name.title')}</h2>
            <p className="helpText">{app.translator.trans('ygpynet-point-system.admin.name.help')}</p>
          </div>
          <Button className="Button Button--primary" onclick={() => app.modal.show(CreateNameDecorationModal, { onCreated: () => this.load() })}>
            <i className="fas fa-plus" /> {app.translator.trans('ygpynet-point-system.admin.name.create')}
          </Button>
        </div>

        {this.items.length === 0 && <p className="PointSystemAdmin-empty">{app.translator.trans('ygpynet-point-system.admin.name.none')}</p>}
    ...
    return (
      <div className={`PointSystemAdmin-decoCard ${enabled ? '' : 'is-disabled'}`}>
        ...
        <div className="PointSystemAdmin-decoCard-meta">
          <strong>{name}</strong>
          <small>
            {app.translator.trans('ygpynet-point-system.admin.name.preset')}: {preset || '—'} · {price} {pointsLabel(app)}
          </small>
        </div>
        <div className="PointSystemAdmin-decoCard-actions">
          ...
          >
            <i className="fas fa-pen" /> {app.translator.trans('ygpynet-point-system.admin.edit')}
          </Button>
          ...
          >
            <i className="fas fa-gift" /> {app.translator.trans('ygpynet-point-system.admin.grant.action_button')}
          </Button>
          <Button className="Button Button--danger" onclick={() => this.remove(deco)}>
            <i className="fas fa-trash" /> {app.translator.trans('ygpynet-point-system.admin.delete')}
          </Button>
        </div>
      </div>
    );
