...
export default class PendingSubmissionsPanel extends Component {
  ...
  view() {
    ...
    const t = (k: string) => app.translator.trans('ygpynet-point-system.admin.submissions.' + k);
  ...
  renderRow(it: any) {
    const t = (k: string, v?: any) => app.translator.trans('ygpynet-point-system.admin.submissions.' + k, v);
    const key = `${it.type}:${it.id}`;
    const busy = this.busy[key] ?? null;
    const familyKey = TYPE_LABEL[it.type] || it.type;
    const familyLabel = app.translator.trans('ygpynet-point-system.admin.tabs.' + familyKey);
  ...
  async act(it: any, action: 'approve' | 'reject') {
    ...
    try {
      ...
      app.alerts.show(
        { type: 'success' },
        app.translator.trans('ygpynet-point-system.admin.submissions.' + (action === 'approve' ? 'approved_alert' : 'rejected_alert'), {
          name: it.name,
        })
      );
    } catch (e: any) {
