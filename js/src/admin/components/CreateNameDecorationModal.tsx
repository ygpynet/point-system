...
export default class CreateNameDecorationModal extends Modal {
  ...
  title() {
    return app.translator.trans('ygpynet-point-system.admin.name.create_title');
  }

  content() {
    const draft = this.draft;
    const previewStyle = draft.preset === 'custom' && !String(draft.customCss).includes('{') ? this.parseInlineCss(draft.customCss) : null;
    const t = (k: string) => app.translator.trans('ygpynet-point-system.admin.name.' + k);
        ...
        <div className="Form-group">
          ...
          <select className="FormControl" value={draft.preset} onchange={(e: Event) => (draft.preset = (e.target as HTMLSelectElement).value)}>
            {BUILTIN_PRESETS.map((p) => (
              <option value={p}>{app.translator.trans(`ygpynet-point-system.admin.name.preset_${p}`)}</option>
            ))}
  ...
  async commit() {
    ...
    try {
      ...
      app.alerts.show({ type: 'success' }, app.translator.trans('ygpynet-point-system.admin.name.created'));
      this.hide();
    } catch (e: any) {
