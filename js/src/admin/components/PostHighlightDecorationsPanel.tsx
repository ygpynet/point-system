    ...
    return (
      <div className="PointSystemAdmin-section">
        <div className="PointSystemAdmin-section-header PointSystemAdmin-section-header--withAction">
          <div>
            <h2>{app.translator.trans('ygpynet-point-system.admin.post_hl.title')}</h2>
            <p className="helpText">{app.translator.trans('ygpynet-point-system.admin.post_hl.help')}</p>
          </div>
          <Button
            className="Button Button--primary"
            onclick={() => app.modal.show(CreatePostHighlightDecorationModal, { onCreated: () => this.load() })}
          >
            <i className="fas fa-plus" /> {app.translator.trans('ygpynet-point-system.admin.post_hl.create')}
          </Button>
        </div>

        {this.items.length === 0 ? (
          <p className="PointSystemAdmin-empty">{app.translator.trans('ygpynet-point-system.admin.post_hl.none')}</p>
        ) : (
                  ...
                  <div className="PointSystemAdmin-card-actions">
                    ...
                    <Button className="Button" onclick={() => this.toggle(it)}>
                      ...
                      {it.attribute('isEnabled')
                        ? app.translator.trans('ygpynet-point-system.admin.disable')
                        : app.translator.trans('ygpynet-point-system.admin.enable')}
                    </Button>
                    <Button
                      className="Button"
                      onclick={() =>
