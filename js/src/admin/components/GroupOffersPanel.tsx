...
export default class GroupOffersPanel extends Component {
    ...
    return (
      <div className="PointSystemAdmin-section">
        <div className="PointSystemAdmin-section-header">
          <h2>{app.translator.trans('ygpynet-point-system.admin.groups.title')}</h2>
          <p className="helpText">{app.translator.trans('ygpynet-point-system.admin.groups.help')}</p>
        </div>

        <div className="PointSystemAdmin-uploader">
          <h3>{app.translator.trans('ygpynet-point-system.admin.groups.create')}</h3>
          <div className="Form-group">
            <label>{app.translator.trans('ygpynet-point-system.admin.groups.field_group')}</label>
            <select
              className="FormControl"
              value={this.draft.groupId}
              onchange={(e: Event) => (this.draft.groupId = Number((e.target as HTMLSelectElement).value))}
            >
              <option value="0">{app.translator.trans('ygpynet-point-system.admin.groups.choose_group')}</option>
          ...
          <div className="Form-group">
            <label className="checkbox">
              ...
              />{' '}
              {app.translator.trans('ygpynet-point-system.admin.groups.field_is_auto')}
            </label>
            <p className="helpText">{app.translator.trans('ygpynet-point-system.admin.groups.field_is_auto_help')}</p>
          </div>
          {this.draft.isAuto && (
            <div className="Form-group">
              <label>{app.translator.trans('ygpynet-point-system.admin.groups.field_points_required')}</label>
              <input
                type="number"
                min="0"
                className="FormControl"
          ...
          <div className="Form-group">
            <label className="checkbox">
              ...
              />{' '}
              {app.translator.trans('ygpynet-point-system.admin.groups.field_is_purchasable')}
            </label>
            <p className="helpText">{app.translator.trans('ygpynet-point-system.admin.groups.field_is_purchasable_help')}</p>
          </div>
          {this.draft.isPurchasable && (
            <div className="Form-group">
              <label>{app.translator.trans('ygpynet-point-system.admin.groups.field_price')}</label>
              <input
                type="number"
                min="0"
                className="FormControl"
          ...
          <Button
            ...
            onclick={() => this.create()}
          >
            {app.translator.trans('ygpynet-point-system.admin.groups.add')}
          </Button>
        </div>

        <h3>{app.translator.trans('ygpynet-point-system.admin.groups.existing')}</h3>
        {this.items.length === 0 && <p className="PointSystemAdmin-empty">{app.translator.trans('ygpynet-point-system.admin.groups.none')}</p>}
        <table className="PointSystemAdmin-table">
          <thead>
            <tr>
              <th>{app.translator.trans('ygpynet-point-system.admin.groups.col_group')}</th>
              <th>{app.translator.trans('ygpynet-point-system.admin.groups.col_mode')}</th>
              <th>{app.translator.trans('ygpynet-point-system.admin.groups.col_points')}</th>
              <th>{app.translator.trans('ygpynet-point-system.admin.groups.col_price')}</th>
              <th>{app.translator.trans('ygpynet-point-system.admin.groups.col_status')}</th>
              <th></th>
            </tr>
          </thead>
          <tbody>
  ...
  renderRow(offer: any) {
    ...
    return (
      <tr key={id}>
        <td>
          ...
          {!listed && (
            <span className="PointSystemAdmin-tag" style="margin-left:6px">
              {app.translator.trans('ygpynet-point-system.admin.availability.unlisted_tag')}
            </span>
          )}
          {maxClaims != null && (
            <span className="PointSystemAdmin-tag" style="margin-left:6px">
        ...
        <td>
          <label className="checkbox" title={app.translator.trans('ygpynet-point-system.admin.groups.field_is_auto') as string}>
            <input type="checkbox" checked={isAuto} onchange={(e: Event) => offer.save({ isAuto: (e.target as HTMLInputElement).checked })} />{' '}
            <i className="fas fa-bolt" />
          </label>{' '}
          <label className="checkbox" title={app.translator.trans('ygpynet-point-system.admin.groups.field_is_purchasable') as string}>
            <input
              type="checkbox"
        ...
        <td>
          <Button className="Button Button--small" onclick={() => offer.save({ isEnabled: !enabled })}>
            {enabled ? app.translator.trans('ygpynet-point-system.admin.disable') : app.translator.trans('ygpynet-point-system.admin.enable')}
          </Button>
        </td>
        <td>
          <Button
            className="Button Button--small"
            title={app.translator.trans('ygpynet-point-system.admin.availability.legend') as string}
        ...
        <td colSpan={6}>
          ...
          <div className="PointSystemAdmin-availabilityRow-actions">
            <Button className="Button Button--primary" onclick={() => this.commitAvailability(offer)}>
              {app.translator.trans('ygpynet-point-system.admin.save')}
            </Button>
            <Button className="Button" onclick={() => this.cancelAvailability(offer)}>
              {app.translator.trans('ygpynet-point-system.admin.cancel')}
            </Button>
          </div>
        </td>
      </tr>
  ...
  async remove(offer: any) {
    if (!confirm(app.translator.trans('ygpynet-point-system.admin.confirm_delete') as string)) return;
    try {
      await offer.delete();
