...
export default class GrantItemModal extends Modal {
  ...
  title() {
    return app.translator.trans('ygpynet-point-system.admin.grant.title', {
      name: String(this.attrs.itemLabel || ''),
    });
  }

  content() {
    const t = (k: string) => app.translator.trans('ygpynet-point-system.admin.grant.' + k);
  ...
  async submit() {
    ...
    try {
      ...
      if (!match) {
        app.alerts.show({ type: 'error' }, app.translator.trans('ygpynet-point-system.admin.grant.user_not_found'));
        return;
      }
      ...
      app.alerts.show(
        { type: 'success' },
        app.translator.trans('ygpynet-point-system.admin.grant.success', {
