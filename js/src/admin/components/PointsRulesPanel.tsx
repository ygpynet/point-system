...
//
// Section schema:
//   transKey  → maps to ygpynet-point-system.admin.rules.section_<key>_{title,help}
...
export default class PointsRulesPanel extends Component {
    ...
    return (
      <div className="PointSystemAdmin-section">
        <div className="PointSystemAdmin-section-header">
          <h2>{app.translator.trans('ygpynet-point-system.admin.rules.title')}</h2>
          <p className="helpText">{app.translator.trans('ygpynet-point-system.admin.rules.help')}</p>
        </div>
        ...
        <div className="PointSystemAdmin-actions PointSystemAdmin-actions--sticky">
          <Button
            ...
            onclick={() => this.save()}
          >
            <i className="fas fa-save" /> {app.translator.trans('ygpynet-point-system.admin.rules.save')}
          </Button>
          {Object.keys(this.dirty).length > 0 && (
            <span className="PointSystemAdmin-dirty">
              <i className="fas fa-circle" />{' '}
              {app.translator.trans('ygpynet-point-system.admin.rules.unsaved', { count: Object.keys(this.dirty).length })}
            </span>
          )}
        </div>
      </div>
  ...
  renderSection(section: SectionDef) {
    const title = app.translator.trans(`ygpynet-point-system.admin.rules.section_${section.transKey}_title`);
    const help = app.translator.trans(`ygpynet-point-system.admin.rules.section_${section.transKey}_help`);
  ...
  renderField(s: FieldDef) {
    ...
    const label = app.translator.trans(`ygpynet-point-system.admin.rules.${s.transKey}`);
    const help = app.translator.trans(`ygpynet-point-system.admin.rules.${s.transKey}_help`);
