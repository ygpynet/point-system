...
app.initializers.add('ygpynet/point-system', () => {
  ...
  extend(IndexSidebar.prototype, 'navItems', function (items) {
    ...
    items.add(
      ...
      <LinkButton href={app.route('pointSystem.shop')} icon={icon}>
        {app.translator.trans('ygpynet-point-system.forum.nav.shop')}
      </LinkButton>,
  ...
  extend(UserControls, 'moderationControls', function (items, user) {
    ...
    items.add(
      ...
      <Button icon="fas fa-coins" onclick={() => app.modal.show(AwardPointsModal, { user })}>
        {app.translator.trans('ygpynet-point-system.forum.user_controls.award_points')}
      </Button>,
      85
    );
  });
  ...
  extend(UserControls, 'userControls', function (items: any, user: any) {
    ...
    items.add(
      ...
      <Button icon="fas fa-handshake" onclick={() => app.modal.show(TradeModal, { target: user })}>
        {app.translator.trans('ygpynet-point-system.forum.user_controls.trade')}
      </Button>,
      80
    );
  } as any);
  ...
  extend(UserPage.prototype, 'navItems', function (items) {
    ...
    if (isSelf && app.forum.attribute('pointSystemTradeEnabled')) {
    ...
    // ── "Points ledger" tab on the user profile sidebar ──────────────────
    // Visible to the profile owner (their own ledger) AND to actors holding
    // `pointSystem.viewTransactions` (admin/moderator audit — they can open
    // it on anyone's profile). The server enforces the same rule per-request
    // in ListUserTransactionsController; UserTransactionsPage bounces
    // non-owners without the permission as a UX backstop.
    if (isSelf || app.forum.attribute('pointSystemCanViewTransactions')) {
      items.add(
  ...
  extend(SessionDropdown.prototype, 'items', function (items) {
    ...
    items.add(
      ...
      <LinkButton icon="fas fa-tshirt" href={app.route('pointSystem.decorations')}>
        {app.translator.trans('ygpynet-point-system.forum.session_dropdown.my_decorations')}
      </LinkButton>,
      80
    );
    ...
    if (app.forum.attribute('pointSystemTradeEnabled') && app.forum.attribute('pointSystemCanTrade')) {
      items.add(
        ...
        <LinkButton icon="fas fa-handshake" href={app.route('pointSystem.trades')}>
          {app.translator.trans('ygpynet-point-system.forum.session_dropdown.trades')}
        </LinkButton>,
        75
      );
    }
  ...
  extend(CommentPost.prototype, 'actionItems', function (this: any, items: any) {
    ...
    items.add(
      ...
      >
        {app.translator.trans('ygpynet-point-system.forum.post_controls.tip')}
      </Button>,
      15
    );
  });
  ...
  extend(PostControls, 'userControls', function (this: any, items: any, post: any) {
    ...
    items.add(
      ...
      <Button icon="fas fa-list" onclick={() => app.modal.show(PostTipListModal, { post })}>
        {app.translator.trans('ygpynet-point-system.forum.post_controls.tip_list')}
      </Button>,
      15
    );
  });
...
function postTipsSummary(total: number): Mithril.Children {
  ...
  const unit = app.translator.trans('ygpynet-point-system.forum.post.tip_unit');
  const title = app.translator.trans('ygpynet-point-system.forum.post.tip_summary_title', { total: total.toLocaleString(), unit }, true);
