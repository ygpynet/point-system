...
export default class ItemGrantedNotification extends Notification {
  ...
  content() {
    ...
    const adminName = fromUser?.displayName?.() || app.translator.trans('ygpynet-point-system.forum.notifications.admin_fallback');

    const typeKey = String(data?.itemType || '').replace(/_decoration$/, '');
    const familyLabel = app.translator.trans(`ygpynet-point-system.forum.notifications.item_family_${typeKey}`) || typeKey;

    return app.translator.trans('ygpynet-point-system.forum.notifications.item_granted', {
      admin: adminName,
      family: familyLabel,
