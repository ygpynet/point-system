...
export default class TradeModal extends Modal {
  ...
  title() {
    const t = (k: string, v?: any) => app.translator.trans('ygpynet-point-system.forum.trade.' + k, v);
  ...
  async finalize() {
    ...
    try {
      ...
      if (this.trade?.status === 'completed') {
        ...
        app.alerts.show({ type: 'success' }, app.translator.trans('ygpynet-point-system.forum.trade.completed_alert'));
      }
    } catch (e: any) {
      ...
      if (code === 'recipient_already_owns_item') {
        this.err = app.translator.trans('ygpynet-point-system.forum.trade.error_recipient_owns_item') as string;
      } else if (code === 'item_equipped') {
        this.err = app.translator.trans('ygpynet-point-system.forum.trade.error_item_equipped') as string;
      } else if (code === 'not_both_accepted') {
        this.err = app.translator.trans('ygpynet-point-system.forum.trade.error_not_both_accepted') as string;
      } else if (code === 'initiator_insufficient_points' || code === 'recipient_insufficient_points') {
        this.err = app.translator.trans('ygpynet-point-system.forum.trade.error_insufficient_points') as string;
      } else if (code === 'item_unavailable') {
        this.err = app.translator.trans('ygpynet-point-system.forum.trade.error_item_unavailable') as string;
      } else {
  ...
  view() {
    ...
    if (this.err === 'no_target') {
      return <div className="Modal-body">{app.translator.trans('ygpynet-point-system.forum.trade.error_no_target')}</div>;
    }
    if (!this.trade) {
      return <div className="Modal-body">{app.translator.trans('ygpynet-point-system.forum.trade.error_load')}</div>;
    }
  ...
  renderItemPicker(yourItems: OfferItem[], owner?: { id?: number; username?: string; displayName?: string; avatarUrl?: string | null }) {
    ...
    return (
      <div className="PointSystemTradeModal-picker">
        {candidates.length === 0 ? (
          <p className="helpText">{app.translator.trans('ygpynet-point-system.forum.trade.picker_empty')}</p>
        ) : (
  ...
  async toggleAccept() {
    ...
    try {
      ...
      if (!wasAccepted && nowAccepted && !theirAccept) {
        app.alerts.show({ type: 'success' }, app.translator.trans('ygpynet-point-system.forum.trade.accepted_waiting_alert'));
        this.hide();
      }
    } catch (e: any) {
  ...
  async cancel() {
    if (!this.trade) return;
    if (!confirm(app.translator.trans('ygpynet-point-system.forum.trade.confirm_cancel') as string)) return;
    this.busy = 'cancel';
    m.redraw();
    try {
      const apiUrl = (app.forum.attribute('apiUrl') || '/api').replace(/\/+$/, '');
