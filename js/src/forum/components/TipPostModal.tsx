...
export default class TipPostModal extends Modal {
  ...
  title() {
    return app.translator.trans('ygpynet-point-system.forum.tip_modal.title');
  }

  content() {
    const t = (k: string) => app.translator.trans('ygpynet-point-system.forum.tip_modal.' + k);
  ...
  async submit() {
    ...
    try {
      ...
      app.alerts.show(
        { type: 'success' },
        app.translator.trans('ygpynet-point-system.forum.tip_modal.success', { amount: amount.toLocaleString() })
      );
      this.hide();
    } catch (e: any) {
      app.alerts.show({ type: 'error' }, e?.response?.errors?.[0]?.detail || app.translator.trans('ygpynet-point-system.forum.tip_modal.error'));
    } finally {
      this.busy = false;
      m.redraw();
    }
