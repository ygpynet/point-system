...
export default class StartTradeModal extends Modal {
  ...
  title() {
    return app.translator.trans('ygpynet-point-system.forum.trades_page.start_title');
  }
  ...
  content() {
    const t = (k: string) => app.translator.trans('ygpynet-point-system.forum.trades_page.' + k);
      ...
      <div className="Modal-body">
        ...
        <div className="Form-group PointSystemStartTradeModal-actions">
          <Button className="Button" onclick={() => this.hide()}>
            {app.translator.trans('ygpynet-point-system.forum.trades_page.start_cancel')}
          </Button>
          <Button className="Button Button--primary" loading={this.busy} disabled={!this.selected || this.busy} onclick={() => this.submit()}>
            <i className="fas fa-handshake" /> {app.translator.trans('ygpynet-point-system.forum.trades_page.start_submit')}
          </Button>
        </div>
      </div>
    );
  ...
  async search(q: string) {
    try {
      ...
      this.suggestions = [];
      const detail = e?.response?.errors?.[0]?.detail || app.translator.trans('ygpynet-point-system.forum.trades_page.start_search_failed');
  ...
  async submit() {
    if (!this.selected) {
      ...
      try {
        ...
        if (!match) {
          this.err = app.translator.trans('ygpynet-point-system.forum.trades_page.start_not_found') as string;
          return;
        }
        this.selected = match;
      } catch (e: any) {
    ...
    if (me && Number(me.id?.()) === Number(this.selected.id?.())) {
      this.err = app.translator.trans('ygpynet-point-system.forum.trade.error_cannot_trade_with_self') as string;
      m.redraw();
      return;
    }
