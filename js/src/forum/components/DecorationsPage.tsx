...
export default class DecorationsPage extends Page {
  ...
  oninit(vnode: any) {
    ...
    const title = app.translator.trans('ygpynet-point-system.forum.my_decorations.title') as string;
    ...
    return (
      <div className="PointSystemDecorations container">
        <div className="PointSystemDecorations-pageHeader">
          <h1>{app.translator.trans('ygpynet-point-system.forum.my_decorations.title')}</h1>
          <div className="PointSystemDecorations-pageHeader-actions">
            ...
            {app.forum.attribute('pointSystemCanViewShop') && (
              <LinkButton href={app.route('pointSystem.shop')} icon="fas fa-arrow-left" className="Button Button--link PointSystemDecorations-back">
                {app.translator.trans('ygpynet-point-system.forum.my_decorations.back_to_shop')}
              </LinkButton>
            )}
            {canSubmit && (
              <Button
              ...
              >
                <i className="fas fa-paper-plane" /> {app.translator.trans('ygpynet-point-system.forum.my_decorations.submit_cta')}
              </Button>
            )}
          </div>
        </div>
        ...
        <nav className="PointSystemDecorations-nav App-titleControl">
          <SelectDropdown
            ...
            accessibleToggleLabel={app.translator.trans('ygpynet-point-system.forum.my_decorations.toggle_nav_label')}
          >
        ...
        {this.tab === 'all' && (
          <section>
            <h2>{app.translator.trans('ygpynet-point-system.forum.my_decorations.tab_all')}</h2>
            {ownedAvatars.length + ownedNames.length + ownedCovers.length + ownedTitles.length + ownedPostHls.length === 0 && (
              <p className="PointSystemDecorations-empty">{app.translator.trans('ygpynet-point-system.forum.my_decorations.none')}</p>
            )}

            {ownedAvatars.length > 0 && avatarEnabled && (
              <div className="PointSystemDecorations-allGroup">
                <h3>{app.translator.trans('ygpynet-point-system.forum.my_decorations.avatar')}</h3>
                    ...
                    <div className={`PointSystemDecorations-item ${equippedAvatarId === d.id ? 'is-equipped' : ''}`} key={`all-av-${d.id}`}>
                      ...
                      <div className="PointSystemDecorations-item-actions">
                        {equippedAvatarId === d.id ? (
                          ...
                          >
                            {app.translator.trans('ygpynet-point-system.forum.my_decorations.unequip')}
                          </Button>
                        ) : (
                          <Button
                            className="Button Button--primary"
                          ...
                          >
                            {app.translator.trans('ygpynet-point-system.forum.my_decorations.equip')}
                          </Button>
                        )}
                      </div>
                    </div>
            ...
            {ownedNames.length > 0 && nameEnabled && (
              <div className="PointSystemDecorations-allGroup">
                <h3>{app.translator.trans('ygpynet-point-system.forum.my_decorations.name')}</h3>
                <div className="PointSystemDecorations-grid">
                  {ownedNames.map((d) => {
            ...
            {ownedCovers.length > 0 && coverEnabled && (
              <div className="PointSystemDecorations-allGroup">
                <h3>{app.translator.trans('ygpynet-point-system.forum.my_decorations.cover')}</h3>
            ...
            {ownedTitles.length > 0 && titleEnabled && (
              <div className="PointSystemDecorations-allGroup">
                <h3>{app.translator.trans('ygpynet-point-system.forum.my_decorations.custom_title')}</h3>
                <div className="PointSystemDecorations-grid">
                  {ownedTitles.map((d) => {
                    const slug = String(d.slug || '').replace(/[^a-zA-Z0-9_-]/g, '');
            ...
            {ownedPostHls.length > 0 && postHlEnabled && (
              <div className="PointSystemDecorations-allGroup">
                <h3>{app.translator.trans('ygpynet-point-system.forum.my_decorations.post_hl')}</h3>
                <div className="PointSystemDecorations-grid">
                  {ownedPostHls.map((d) => {
                    const slug = String(d.slug || '').replace(/[^a-zA-Z0-9_-]/g, '');
        ...
        {this.tab === 'avatar' && avatarEnabled && (
          <section>
            <h2>{app.translator.trans('ygpynet-point-system.forum.my_decorations.avatar')}</h2>
        ...
        {this.tab === 'name' && nameEnabled && (
          <section>
            <h2>{app.translator.trans('ygpynet-point-system.forum.my_decorations.name')}</h2>
            {ownedNames.length === 0 && (
              <p className="PointSystemDecorations-empty">{app.translator.trans('ygpynet-point-system.forum.my_decorations.none')}</p>
            )}
            <div className="PointSystemDecorations-grid">
        ...
        {this.tab === 'title' && titleEnabled && (
          <section>
            <h2>{app.translator.trans('ygpynet-point-system.forum.my_decorations.custom_title')}</h2>
        ...
        {this.tab === 'post-hl' && postHlEnabled && (
          <section>
            <h2>{app.translator.trans('ygpynet-point-system.forum.my_decorations.post_hl')}</h2>
        ...
        {this.tab === 'cover' && coverEnabled && (
          <section>
            <h2>{app.translator.trans('ygpynet-point-system.forum.my_decorations.cover')}</h2>
  ...
  navItems(avatarEnabled: boolean, nameEnabled: boolean, coverEnabled: boolean, titleEnabled: boolean, postHlEnabled: boolean) {
    ...
    return [
      item('all', 'fas fa-th-large', t('ygpynet-point-system.forum.my_decorations.tab_all') as string),
      avatarEnabled ? item('avatar', 'fas fa-user-circle', t('ygpynet-point-system.forum.my_decorations.avatar') as string) : null,
      nameEnabled ? item('name', 'fas fa-font', t('ygpynet-point-system.forum.my_decorations.name') as string) : null,
      coverEnabled ? item('cover', 'fas fa-image', t('ygpynet-point-system.forum.my_decorations.cover') as string) : null,
      titleEnabled ? item('title', 'fas fa-id-badge', t('ygpynet-point-system.forum.my_decorations.custom_title') as string) : null,
      postHlEnabled ? item('post-hl', 'fas fa-highlighter', t('ygpynet-point-system.forum.my_decorations.post_hl') as string) : null,
    ];
  }
