...
export default class AwardPointsModal extends Modal {
  ...
  title() {
    return app.translator.trans('ygpynet-point-system.forum.award_modal.title', {
      name: this.attrs.user.displayName(),
    });
  }

  content() {
    const t = (k: string) => app.translator.trans('ygpynet-point-system.forum.award_modal.' + k);
  ...
  async submit(direction: 'add' | 'remove') {
    ...
    try {
      ...
      app.alerts.show(
        { type: 'success' },
        app.translator.trans(direction === 'add' ? 'ygpynet-point-system.forum.award_modal.added' : 'ygpynet-point-system.forum.award_modal.removed', {
          name: this.attrs.user.displayName(),
