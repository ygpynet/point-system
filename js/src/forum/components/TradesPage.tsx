...
export default class TradesPage extends Page {
  ...
  oninit(vnode: any) {
    ...
    const title = app.translator.trans('ygpynet-point-system.forum.trades_page.title') as string;
  ...
  async load() {
    ...
    try {
      ...
      this.err = e?.response?.errors?.[0]?.detail || (app.translator.trans('ygpynet-point-system.forum.trades_page.load_failed') as string);
    } finally {
      this.loading = false;
      m.redraw();
    }
  ...
  view() {
    ...
    const t = (k: string, v?: any) => app.translator.trans('ygpynet-point-system.forum.trades_page.' + k, v);
