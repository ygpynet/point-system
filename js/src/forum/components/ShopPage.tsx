...
export default class ShopPage extends Page {
  ...
  oninit(vnode: any) {
    ...
    const title = app.translator.trans('ygpynet-point-system.forum.shop.title') as string;
    ...
    if (!app.forum.attribute('pointSystemCanViewShop')) {
      return (
        <div className="PointSystemShop-notFound container">
          <h1>404</h1>
          <p>{app.translator.trans('ygpynet-point-system.forum.shop.not_found')}</p>
        </div>
      );
    }
    ...
    return (
      <div className="PointSystemShop">
        <div className="PointSystemShop-header container">
          <h1>{app.translator.trans('ygpynet-point-system.forum.shop.title')}</h1>
          <p className="PointSystemShop-subtitle">{app.translator.trans('ygpynet-point-system.forum.shop.subtitle')}</p>
          {user && (
            <div className="PointSystemShop-balance">
              <span className="PointSystemShop-balance-item">
                ...
                <span>
                  {app.translator.trans('ygpynet-point-system.forum.shop.your_balance')}:{' '}
              ...
              {app.forum.attribute('pointSystem.lifetime_enabled') !== false && (
                <span className="PointSystemShop-balance-item PointSystemShop-balance-item--lifetime">
                  ...
                  <span>
                    {app.translator.trans('ygpynet-point-system.forum.shop.lifetime')}:{' '}
              ...
              <LinkButton href={app.route('pointSystem.decorations')} icon="fas fa-tshirt" className="Button Button--link">
                {app.translator.trans('ygpynet-point-system.forum.shop.my_decorations')}
              </LinkButton>
            </div>
          )}
        </div>
        ...
        <nav className="PointSystemShop-nav App-titleControl">
          <SelectDropdown
            ...
            accessibleToggleLabel={app.translator.trans('ygpynet-point-system.forum.shop.toggle_nav_label')}
          >
        ...
        {this.tab === 'tiers' ? (
          ...
          <div className="container">
            {items.length === 0 ? (
              <div className="PointSystemShop-empty">{app.translator.trans('ygpynet-point-system.forum.shop.empty')}</div>
            ) : (
              [
  ...
  navItems(avatarEnabled: boolean, nameEnabled: boolean, coverEnabled: boolean, titleEnabled: boolean, postHlEnabled: boolean, hasTiers: boolean) {
    ...
    return [
      avatarEnabled ? item('avatar', 'fas fa-user-circle', t('ygpynet-point-system.forum.shop.tab_avatar') as string) : null,
      nameEnabled ? item('name', 'fas fa-font', t('ygpynet-point-system.forum.shop.tab_name') as string) : null,
      coverEnabled ? item('cover', 'fas fa-image', t('ygpynet-point-system.forum.shop.tab_cover') as string) : null,
      titleEnabled ? item('title', 'fas fa-id-badge', t('ygpynet-point-system.forum.shop.tab_title') as string) : null,
      postHlEnabled ? item('post-hl', 'fas fa-highlighter', t('ygpynet-point-system.forum.shop.tab_post_hl') as string) : null,
      hasTiers ? item('tiers', 'fas fa-layer-group', t('ygpynet-point-system.forum.shop.tab_tiers') as string) : null,
    ];
  }
  ...
  renderTiers(user: any) {
    ...
    if (!offers.length) {
      return (
        <div className="container">
          <div className="PointSystemShop-empty">{app.translator.trans('ygpynet-point-system.forum.shop.tiers_empty')}</div>
        </div>
      );
    }
    ...
    return (
      <div className="container PointSystemShop-tiers">
        <p className="PointSystemShop-subtitle">{app.translator.trans('ygpynet-point-system.forum.shop.tiers_help')}</p>
        ...
        <div className="PointSystemShop-tier-body">
          ...
          {isAuto && (
            <div className="PointSystemShop-tier-points PointSystemShop-tier-points--threshold">
              ...
              {app.translator.trans('ygpynet-point-system.forum.shop.tier_threshold')}
            </div>
          )}
          {isPurchasable && (
            <div className="PointSystemShop-tier-points PointSystemShop-tier-points--price">
              <i className={currencyIcon} /> {Number(price).toLocaleString()} {app.translator.trans('ygpynet-point-system.forum.shop.tier_buy_cost')}
            </div>
          )}
        ...
        <div className="PointSystemShop-tier-action">
          ...
          {user && owned && (
            <Button className="Button Button--primary PointSystemShop-equippedBtn" disabled>
              <i className="fas fa-check" /> {app.translator.trans('ygpynet-point-system.forum.shop.tier_claimed')}
            </Button>
          )}
          ...
          {user && !owned && qualifiedByLifetime && (
            <Button className="Button Button--primary" loading={isClaiming} onclick={() => this.confirmClaimOffer(offer, 'auto')}>
              <i className="fas fa-bolt" /> {app.translator.trans('ygpynet-point-system.forum.shop.tier_claim')}
            </Button>
          )}
          ...
          {user && !owned && !qualifiedByLifetime && isPurchasable && canBuy && (
            <Button className="Button Button--primary" loading={isClaiming} onclick={() => this.confirmClaimOffer(offer, 'purchase')}>
              <i className="fas fa-coins" /> {app.translator.trans('ygpynet-point-system.forum.shop.tier_buy')}
            </Button>
          )}
          ...
          {user && !owned && !qualifiedByLifetime && (!isPurchasable || !canBuy) && (
            <Button className="Button" disabled>
              {isPurchasable && !canBuy
                ? app.translator.trans('ygpynet-point-system.forum.shop.tier_not_enough')
                : app.translator.trans('ygpynet-point-system.forum.shop.tier_locked')}
            </Button>
          )}
        </div>
      </div>
  ...
  async claimTier(offer: any, mode: 'auto' | 'purchase' = 'purchase') {
    ...
    try {
      ...
      app.alerts.show({ type: 'success' }, app.translator.trans('ygpynet-point-system.forum.shop.tier_claimed_alert', { name: offer.groupName }));
    } catch (e: any) {
      const detail = e?.response?.errors?.[0]?.detail || app.translator.trans('ygpynet-point-system.forum.shop.tier_claim_failed');
      ...
      <div className={cardCls}>
        {equipped && (
          <div className="PointSystemShop-card-equippedRibbon">
            <i className="fas fa-check-circle" /> {app.translator.trans('ygpynet-point-system.forum.shop.equipped_label')}
          </div>
        )}
        ...
        <div className="PointSystemShop-card-body">
          ...
          {item.creatorUsername && (
            <a
              ...
              title={String(
                app.translator.trans('ygpynet-point-system.forum.shop.creator_tooltip', {
            ...
            >
              ...
              <span>
                {app.translator.trans('ygpynet-point-system.forum.shop.by_creator', {
                  name: item.creatorDisplayName || item.creatorUsername,
                })}
              </span>
            </a>
          ...
          {user && equipped && (
            <Button className="Button Button--primary PointSystemShop-equippedBtn" disabled>
              <i className="fas fa-check" /> {app.translator.trans('ygpynet-point-system.forum.shop.equipped')}
            </Button>
          )}

          {user && owned && !equipped && (
            <Button className="Button Button--primary" loading={isClaiming} onclick={() => this.equip(item)}>
              <i className="fas fa-bolt" /> {app.translator.trans('ygpynet-point-system.forum.shop.equip')}
            </Button>
          )}
          ...
          {user && !owned && (
            <Button className="Button Button--primary" disabled={!canAfford} loading={isClaiming} onclick={() => this.confirmClaim(item)}>
              {canAfford
                ? app.translator.trans('ygpynet-point-system.forum.shop.claim')
                : app.translator.trans('ygpynet-point-system.forum.shop.not_enough')}
            </Button>
          )}
        </div>
      </div>
  ...
  truncationNote(shown: number) {
    ...
    return (
      <p className="PointSystemShop-truncationNote">
        <i className="fas fa-circle-info" /> {app.translator.trans('ygpynet-point-system.forum.shop.truncated', { count: limit })}
      </p>
    );
  }
  ...
  guestLoginButton() {
    return (
      <Tooltip text={app.translator.trans('ygpynet-point-system.forum.shop.must_login') as string}>
        <Button className="Button PointSystemShop-loginBtn" onclick={() => showLogInModal()}>
          <i className="fas fa-right-to-bracket" /> {app.translator.trans('ygpynet-point-system.forum.shop.login_to_claim')}
        </Button>
      </Tooltip>
    );
  }
  ...
  availabilityBadges(item: ShopItem): any[] {
    ...
    if (item.maxClaims != null && item.maxClaims > 0) {
      ...
      badges.push(
        <span className={`PointSystemShop-card-badge ${cls}`}>
          <i className="fas fa-box" /> {t('ygpynet-point-system.forum.shop.badge_remaining', { count: remaining })}
        </span>
      );
    }
    ...
    if (item.availableUntil) {
      ...
      if (remainingMs > 0) {
        ...
        const label =
          days >= 2
            ? t('ygpynet-point-system.forum.shop.badge_ends_in_days', { count: days })
            : t('ygpynet-point-system.forum.shop.badge_ends_in_hours', { count: Math.max(1, hours) });
    ...
    if (item.availableFrom && new Date(item.availableFrom).getTime() > Date.now()) {
      ...
      badges.push(
        <span className="PointSystemShop-card-badge is-future">
          <i className="fas fa-calendar" /> {t('ygpynet-point-system.forum.shop.badge_starts_on', { date: dateLabel })}
        </span>
      );
    }
  ...
  confirmClaim(item: ShopItem) {
    ...
    app.modal.show(ConfirmPurchaseModal, {
      title: app.translator.trans('ygpynet-point-system.forum.confirm.title'),
      itemName: item.name,
      itemPrice: item.price,
      currentBalance: balance,
      preview,
      confirmLabel: app.translator.trans('ygpynet-point-system.forum.shop.claim'),
  ...
  confirmClaimOffer(offer: any, mode: 'auto' | 'purchase') {
    ...
    app.modal.show(ConfirmPurchaseModal, {
      title: app.translator.trans(isAuto ? 'ygpynet-point-system.forum.confirm.title_tier' : 'ygpynet-point-system.forum.confirm.title_purchase'),
      itemName: offer.groupName || '—',
      itemPrice: cost,
      currentBalance: balance,
      preview,
      confirmLabel: app.translator.trans(isAuto ? 'ygpynet-point-system.forum.shop.tier_claim' : 'ygpynet-point-system.forum.shop.tier_buy'),
  ...
  async claim(item: ShopItem) {
    const user = app.session.user;
    if (!user) {
      app.alerts.show({ type: 'error' }, app.translator.trans('ygpynet-point-system.forum.shop.must_login'));
      return;
    }
    ...
    try {
      ...
      app.alerts.show({ type: 'success' }, app.translator.trans('ygpynet-point-system.forum.shop.claimed', { name: item.name }));
    } catch (e: any) {
      const detail = e?.response?.errors?.[0]?.detail || app.translator.trans('ygpynet-point-system.forum.shop.claim_failed');
      app.alerts.show({ type: 'error' }, detail);
    } finally {
      this.claiming.delete(key);
      m.redraw();
  ...
  async equip(item: ShopItem) {
    ...
    try {
      ...
      app.alerts.show({ type: 'success' }, app.translator.trans('ygpynet-point-system.forum.shop.equipped_alert', { name: item.name }));
    } catch (e: any) {
      const detail = e?.response?.errors?.[0]?.detail || app.translator.trans('ygpynet-point-system.forum.shop.equip_failed');
      app.alerts.show({ type: 'error' }, detail);
    } finally {
      this.claiming.delete(key);
      m.redraw();
