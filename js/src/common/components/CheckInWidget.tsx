...
export function createCheckInWidget(WidgetBase) {
  return class CheckInWidget extends WidgetBase {
    ...
    title() {
      const app = forumApp();
      return app
        ? app.translator.trans('ramon-point-system.forum.checkin.widget_title')
        : 'Daily check-in';
    }
        ...
        <div className="PointSystemCheckInWidget-body">
          ...
          {done && rank ? (
            <div className="PointSystemCheckInWidget-rank">
              ...
              {app.translator.trans('ramon-point-system.forum.checkin.rank_today', { rank })}
            </div>
          ) : null}

          {!done && todayTotal > 0 && (
            <div className="PointSystemCheckInWidget-todayCount">
              {app.translator.trans('ramon-point-system.forum.checkin.today_total', { count: todayTotal })}
            </div>
          )}

          {!done && nextReward > 0 && (
            <div className="PointSystemCheckInWidget-reward">
              {app.translator.trans('ramon-point-system.forum.checkin.reward_today', { amount: nextReward })}
            </div>
          )}

          {!done && perDayExtra > 0 && capDays > 0 && (
            <div className="PointSystemCheckInWidget-next">{app.translator.trans('ramon-point-system.forum.checkin.growth_hint', { days: capDays })}</div>
          )}
          ...
          >
            {done ? app.translator.trans('ramon-point-system.forum.checkin.done') : app.translator.trans('ramon-point-system.forum.checkin.action')}
          </Button>
      ...
      try {
        ...
        app.alerts.show(
          ...
          app.translator.trans(
            withRank ? 'ramon-point-system.forum.checkin.success_rank' : 'ramon-point-system.forum.checkin.success',
            { amount: res?.data?.awarded, rank: res?.data?.rank, streak: res?.data?.streak }
          )
        );
      } catch (error) {
        app.alerts.show({ type: 'error' }, app.translator.trans('ramon-point-system.forum.checkin.error'));
      } finally {
        this.loading = false;
        m.redraw();
      }
    ...
    async makeUp() {
      ...
      try {
        ...
        app.alerts.show(
          { type: 'success' },
          app.translator.trans('ramon-point-system.forum.checkin.makeup_success', {
            cost: res?.data?.paid,
        ...
        const key =
          code === 'makeup_limit'
            ? 'ramon-point-system.forum.checkin.makeup_error_limit'
            : code === 'insufficient_balance'
              ? 'ramon-point-system.forum.checkin.makeup_error_insufficient'
              : 'ramon-point-system.forum.checkin.error';
